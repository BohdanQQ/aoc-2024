use std::{sync::{atomic, Arc}, thread};
...
enum Pos {
    Air, Obstacle, Guard, Visited(i8, i8)
}
...
fn do_step(field: &mut [Vec<Pos>], row: &mut usize, col: &mut usize, dir_row: &mut i8, dir_col: &mut i8) -> Option<u32> {
...
// part one, does not use visited, uses guard of Pos
// mutate the field, position, return nr of tiles changed
fn do_step_2(field: &mut [Vec<Pos>], row: &mut usize, col: &mut usize, dir_row: &mut i8, dir_col: &mut i8) -> Option<bool> {
    let next_r = *row as i64 + *dir_row as i64;
    let next_c = *col as i64 + *dir_col as i64;
    if next_r < 0 || next_c <0 || next_c as usize >= field.len() || next_r as usize >= field.len() {
        return Some(false);
...
pub fn part_one(input: &str) -> Option<u32> {
    let mut parsed = input.split_ascii_whitespace().map(|l| l.chars().map(|c| if c == '#' { Pos::Obstacle } else if c == '.' { Pos::Air } else { Pos::Guard }).collect::<Vec<_>>()).collect::<Vec<_>>();
    let mut guard = (0, 0);
...
pub fn part_two(input: &str) -> Option<u32> {
    let parsed = input.split_ascii_whitespace().map(|l| l.chars().map(|c| if c == '#' { Pos::Obstacle } else if c == '.' { Pos::Air } else { Pos::Guard }).collect::<Vec<_>>()).collect::<Vec<_>>();
